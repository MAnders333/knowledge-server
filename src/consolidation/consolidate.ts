        ...
        try {
          await this.reconsolidate(entry, sessionIds, entriesMap, {
            ...
            onUpdate: (id, updated) => {
              ...
              chunkUpdated++;
              changedIds.add(id);
              // Update the cache with the new content but mark embedding absent.
              // This prevents a later extracted entry in this chunk from matching
              // the stale (embedding-cleared) version, while still keeping the
              // updated entry visible for contradiction scanning via its content/topics.
              // The embedding will be regenerated by ensureEmbeddings at end of run.
