...
- It contains user- or machine-specific details that would not be useful to other users or on a different machine. This includes: local filesystem paths (e.g. "/Users/alice/projects/foo"), usernames or system account names, personal credentials, individual tool configs, or any detail that identifies a specific person's environment rather than a shared system. Such details belong in the most individual/personal domain available (see DOMAIN ASSIGNMENT). If no such domain exists, encode the portable concept (e.g. "the project lives in a local clone of repo X", "the service connects as a dedicated DB user") but omit the identity-specific detail (e.g. the literal path or username).
