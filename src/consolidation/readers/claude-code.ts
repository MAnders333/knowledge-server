...
import type { Episode, EpisodeMessage, IEpisodeReader, ProcessedRange } from "../../types.js";
...
interface ContentPart {
  ...
  // Other fields (id, input, etc.) ignored — we only need text and tool results
}
...
export class ClaudeCodeEpisodeReader implements IEpisodeReader {
   ...
   */
  private loadModifiedSessions(afterMessageTimeCreated: number): ParsedSession[] {
   ...
   */
  private parseSessionFile(filePath: string, sessionId: string): ParsedSession | null {
    let content: string;
    try {
    ...
    for (const line of lines) {
      ...
      if (record.timestamp && (record.type === "user" || record.type === "assistant")) {
    ...
    return {
      ...
      mtimeMs,
      records,
      minTimestampMs: minTimestampMs === Number.MAX_SAFE_INTEGER ? 0 : minTimestampMs,
      maxTimestampMs,
    };
  }
   ...
   */
  private segmentSession(session: ParsedSession, processedRanges: ProcessedRange[]): Episode[] {
  ...
  private getCompactionPoints(session: ParsedSession): Array<{
    ...
    for (let i = 0; i < session.records.length; i++) {
      ...
      for (let j = i + 1; j < session.records.length; j++) {
        ...
        const summaryTimestampMs = next.timestamp ? Date.parse(next.timestamp) : 0;
        ...
        if (summaryText.trim()) {
          points.push({ compactionTimestamp, summaryText, summaryUuid, summaryTimestampMs });
        }
  ...
  private segmentWithCompactions(
    ...
    const tailMessages = this.getMessagesAfterCompaction(session, lastCompaction.summaryUuid);
   ...
   */
  private extractMessage(rec: CCRecord): EpisodeMessage | null {
      ...
      if (rec.type === "user" && includeToolOutputs.length > 0) {
        ...
        const relevant = toolResults
          .map((p) => {
            ...
            return { name: p.name ?? "", text: resultText };
          })
          .filter((r) => r.name && r.text && includeToolOutputs.includes(r.name));

        if (relevant.length > 0) {
          toolContent = relevant
            .map((r) => {
              const truncated = r.text.length > MAX_TOOL_OUTPUT_CHARS
   ...
   */
  private chunkByTokenBudget(messages: EpisodeMessage[], maxTokens: number): EpisodeMessage[][] {
