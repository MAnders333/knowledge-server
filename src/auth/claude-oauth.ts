...
/** User-Agent that Anthropic's OAuth-gated endpoint expects. */
...
/** Returns true when valid-looking OAuth tokens exist on disk. */
...
export async function refreshTokens(refreshToken: string): Promise<OAuthTokens> {
	const response = await fetch(ANTHROPIC_TOKEN_URL, {
		...
		headers: {
			...
			// Anthropic's OAuth token endpoint (platform.claude.com) requires this
			// specific User-Agent. It mirrors what the official Claude CLI sends and
			// was determined empirically. If requests start failing with 4xx, verify
			// whether Anthropic has changed the expected value.
	...
	return {
		...
		// Some responses omit refresh_token (the existing one stays valid).
 ...
 * refresh request is made at a time. Anthropic's token endpoint uses rotating
 * refresh tokens (RFC 6749 §10.4): a second concurrent exchange would fail
 * because the first exchange invalidates the refresh token.
 */
let _refreshPromise: Promise<OAuthTokens> | null = null;

/**
 * Return a fresh access token, auto-refreshing from disk if the stored token
 * is about to expire (within 60 s). Updated tokens are written back to disk.
 *
 * Concurrent callers that arrive while a refresh is in progress share the same
 * in-flight promise rather than each issuing a parallel exchange request.
 *
...
export async function getAccessToken(): Promise<string> {
	...
	// Deduplicate concurrent refreshes — only one exchange request at a time.
	if (!_refreshPromise) {
 ...
 * server to capture the callback, then exchanges the code for tokens.
 *
...
export async function runOAuthFlow(): Promise<OAuthTokens> {
	...
	// Box object so the server handler closure can reference the timeout handle
	// that is assigned after server.listen() (avoids a let-only-assigned-once lint error).
	...
	const response = await fetch(ANTHROPIC_TOKEN_URL, {
		...
		headers: {
			"Content-Type": "application/json",
			// Same User-Agent as refreshTokens() — required by the token endpoint.
			"User-Agent": "axios/1.13.6",
		},
		body: JSON.stringify({
			code,
